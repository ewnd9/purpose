...
import {ONGOING, COMPLETED, BACKLOG, DELETED} from '../../modules/items/items-reducer';
...
export const Navigation = ({
  ...
  <nav>
    ...
    <div className="overflow-x-auto mt3 flex flex-row">
      ...
      <div>
        <span className="dip f5 gray mr3">
          <span className="pointer" onClick={() => setIsSettingsOpen(!isSettingsOpen)}>
            {isSettingsOpen ? '✔️' : '⚙'}
          </span>
        </span>
    ...
    {(activeProject && activeProject.path && (
      <div className="mt1 gray">
        ...
        <span
          ...
          onClick={() => ipcRenderer.send('OPEN_PROJECT', {path: activeProject.path})}
        >
          {activeProject.path}
        </span>
      </div>
    ...
    {(activeProject && activeProject.plan && (
      <div className="mt1 gray">
        Plan:{' '}
        <span className="underline pointer" onClick={() => ipcRenderer.send('OPEN_PLAN', {plan: activeProject.plan})}>
          {activeProject.plan}
        </span>
      </div>
    )) ||
...
const NavigationFilterItem = ({value, label, activeGroupObject, activeFilter, setActiveFilter}) => (
  <a
    href="#"
  ...
  >
    {`${label} (${((activeGroupObject && activeGroupObject.itemsByFilter[value]) || []).length})`}
  </a>
);
export const Checkbox = ({id, label, value, onChange}) => (
  <React.Fragment>
    <input type="checkbox" className="mr2" id={id} type="checkbox" defaultChecked={value} onChange={onChange} />
