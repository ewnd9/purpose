...
export const ScheduleModalBase = ({
  modalOpts: { id },
  setModalOpened,
  modalOpened,
  scheduledItems,
  ...
  >
    ...
    <div className="mt3 fira-mono">
      {scheduledItems
        .slice(id ? 1 : 0)
        .map(({ date, items, totalEstimate }) => (
          <div key={date}>
            <div>
              <div
                className={cx("dib mv1", { pointer: !!id })}
                onClick={id ? () => onDateSelect(date) : null}
              >
                <span>{"- "}</span>
              ...
              {items.map(item => (
                <div key={item.id} className="ml3">
                  {`- ${getFirstLine(item.text)} (${formatTime(
                    parseScheduleTime(item)
                  )})`}
                </div>
              ))}
            </div>
      ...
      {(id && (
        <div
          className="underline pointer mv1"
